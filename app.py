...
with st.sidebar:
    ...
    random_state = st.slider("Random State (Seed)", 0, 100, 42, 1)
    ...
    st.markdown("""
    <div style="margin-top:2rem; font-family:'Share Tech Mono',monospace; font-size:0.6rem; color:rgba(0,245,255,0.25); letter-spacing:0.15em; line-height:2;">
...
@st.cache_data
def train_models(test_size, random_state, use_lr, use_dt, use_rf, data_hash):
    ...
    if use_rf:
        ...
        results['Random Forest'] = {
            'model': rf, 'preds': preds,
            'accuracy': accuracy_score(y_test, preds),
            'report': classification_report(y_test, preds, output_dict=True, zero_division=0),
            'cm': confusion_matrix(y_test, preds),
            'color': NEON_GREEN
        }
    return results, X_train.shape, X_test.shape, X.columns.tolist()
    ...
    with col_left:
        ...
        for bar, val in zip(bars, counts.values):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + counts.max()*0.01,
                    f'{val:,}', ha='center', va='bottom', fontsize=9, color='#e8f4f8', fontfamily='monospace')
        ax.set_title("CLASS DISTRIBUTION", fontsize=9, letterSpacing=2, pad=12)
    ...
    if not any([use_lr, use_dt, use_rf]):
        ...
        if run_btn or "results" not in st.session_state:
            with st.spinner("🧬 Training models..."):
                ...
                results, train_shape, test_shape, feature_names = train_models(
                    test_size, random_state, use_lr, use_dt, use_rf, data_hash
                )
        ...
        with col1:
            st.metric("Training Samples", f"{int(len(data)*(1-test_size)):,}")
        with col2:
            st.metric("Test Samples", f"{int(len(data)*test_size):,}")
        with col3:
            st.metric("Models Trained", f"{len(results)}")
...
with tab3:
    ...
    if "results" not in st.session_state or not st.session_state["results"]:
        ...
        fig.suptitle("CONFUSION MATRICES", fontsize=10, color=NEON_CYAN, y=1.02, fontfamily='monospace')
        fig.tight_layout()
        st.pyplot(fig)
        plt.close()
...
with tab4:
    ...
    if "results" not in st.session_state or not st.session_state["results"]:
        st.info("▶ Train models first (Run Analysis in sidebar).")
    else:
        results = st.session_state["results"]
        feature_names = st.session_state.get("feature_names", data.drop('Diabetes_012', axis=1).columns.tolist())
        ...
        if predict_btn:
            ...
            for ax, (name, res) in zip(axes, results.items()):
                ...
                ax.grid(axis='y', zorder=0)
                ax.set_axisbelow(True)
            fig.suptitle("PREDICTION PROBABILITY DISTRIBUTION", fontsize=9, color=NEON_CYAN, fontfamily='monospace')
            fig.tight_layout()
            st.pyplot(fig)
            plt.close()
